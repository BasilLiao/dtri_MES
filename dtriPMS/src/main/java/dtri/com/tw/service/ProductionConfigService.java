...
public class ProductionConfigService {
	...
	public PackageBean getData(JSONObject body, int page, int p_size) {
		...
		if (body == null || body.isNull("search")) {
			...
			int ord = 0;
			object_header.put(FFS.ord((ord += 1), FFM.Hmb.H) + "pb_cell", FFS.h_t("SN_系統欄位", "160px", FFM.Wri.W_Y));
			object_header.put(FFS.ord((ord += 1), FFM.Hmb.H) + "pb_value", FFS.h_t("SN_名稱定義", "160px", FFM.Wri.W_Y));
...
//
//			obj_m.put(FFS.h_m(FFM.Dno.D_S,FFS.TTA, FFS.TEXT, "", "", FFM.Wri.W_Y, "col-md-12", false, n_val, "sys_note", "備註"));
//			obj_m.put(FFS.h_m(FFM.Dno.D_S,FFS.INP, FFS.NUMB, "0", "0", FFM.Wri.W_Y, "col-md-2", true, n_val, "sys_sort", "排序"));
//			obj_m.put(FFS.h_m(FFM.Dno.D_S,FFS.INP, FFS.NUMB, "", "", FFM.Wri.W_N, "col-md-2", false, n_val, "sys_ver", "版本"));

			a_val = new JSONArray();
			a_val.put((new JSONObject()).put("value", "正常").put("key", "0"));
			a_val.put((new JSONObject()).put("value", "異常").put("key", "1"));
//			obj_m.put(FFS.h_m(FFM.Dno.D_S,FFS.SEL, FFS.TEXT, "", "0", FFM.Wri.W_Y, "col-md-2", true, a_val, "sys_status", "狀態"));
			bean.setCell_modify(obj_m);

			// 放入包裝(search)
			JSONArray object_searchs = new JSONArray();
			object_searchs.put(FFS.h_s(FFM.Tag.INP, FFM.Type.TEXT, "", "col-md-2", "pb_value", "SN_名稱", n_val));
